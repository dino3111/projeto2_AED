...
Graph* GraphGetSubgraph(const Graph* g, IndicesSet* vertSet) {
  ...
  while (v != -1) {
    ...
    if (ListSearch(g->verticesList, &search_dummy) == 0) {
      struct _Vertex* original_v = (struct _Vertex*)ListGetCurrentItem(g->verticesList);
      List* edges = original_v->edgesList;
      ...
      if (!ListIsEmpty(edges)) {
        ...
        for (int i = 0; i < ListGetSize(edges); i++) {
          ...
          if (IndicesSetContains(vertSet, e->adjVertex)) {
            // If undirected, only add if v < adjVertex to avoid duplicates
            if (g->isDigraph || v < (int)e->adjVertex) {
               if (g->isWeighted) {
                 GraphAddWeightedEdge(new, v, e->adjVertex, e->weight);
               } else {
                 GraphAddEdge(new, v, e->adjVertex);
               }
            }
          }
...
double* GraphComputeVertexWeights(const Graph* g) {
  ...
  if (!ListIsEmpty(vertices)) {
    ...
    for (int i = 0; i < ListGetSize(vertices); i++) {
      ...
      // Sum the weights of all edges adjacent to this vertex
      List* edges = v->edgesList;
      if (!ListIsEmpty(edges)) {
        ListMoveToHead(edges);
        for (int j = 0; j < ListGetSize(edges); j++) {
          struct _Edge* e = (struct _Edge*)ListGetCurrentItem(edges);
          weight += e->weight;  // For unweighted graphs, weight = 1.0 (degree)
          ListMoveToNext(edges);
        }
      }
      weightsArray[v->id] = weight;
